...
const ArticleQteReport = () => {
  ...
  const handleAddDetails = async () => {
    ...
    const data = {
      IdArticle: selectedArticle.value.toString(),
      IdMarque: parseInt(selectedBrand, 10),
      Contenance: parseFloat(newCapacity),
      Qte: parseInt(newQte, 10),
      IdMission: missionId,
    };

    try {
      const response = await axios.post(
        `${API_BASE_URL}RapportQte/AddLigneMarqueQte`,
        data,
      );
  ...
  const renderQte = ({ item }) => (
    <View style={styles.qteRow}>
      ...
      <Text style={styles.qteText}>{item.Qte} Piéces</Text>
    </View>
  );
...
export default ArticleQteReport;
